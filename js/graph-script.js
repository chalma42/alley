...
var options222 = {
  ...
  plugins: [
    ...
    var options2 = {
      ...
      axisY: {
        ...
        showGrid: false,
        // Функция интерполяции метки позволяет менять значение метки,
        // в текущем примере появляются миллионы "млн.".
        labelInterpolationFnc: function(value) {
          if(value % 2 === 0) {
            if(value === 0) return '0';
            else return value;
  ...
  //если ширина браузера больше 576 то options для десктоп, в противном случае options для мобильников
  if (width > 576) {
    var chart = new Chartist.Line('.chart', data, options);
  } else {
    var chart = new Chartist.Line('.chart', data, options2);
  }
  
  //@chalma смена месяца с проверкой на ширину браузер
  ...
  elem.oninput = function () {
    if(elem.value === "2021-07") {
      if (width > 576) {
        ...
        Chartist.Line('.chart', data, options2);
      }
    } else {
      if (width > 576) {
        Chartist.Line('.chart', data2, options);
      } else {
        Chartist.Line('.chart', data2, options2);
      }
    } 
  }
   // изменение ширины экрана в реальном времени
